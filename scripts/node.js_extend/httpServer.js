...
export default class HttpServer {
  ...
  start(rootDir, port) {
    ...
    if (port) {
      if (typeof port === constants.types.NUMBER && port > 0 && port < 65535)
        this.port = port;
      else {
        logger.error('Argument "port" is invalid in HttpServer.start! The value is ', port);
      }
    }
    while (!this.listening) {
      try {
        let server = http.createServer(this._reqResp.bind(this));
        server.listen(this.port);
      } catch (err) {
        logger.warn("The port ", this.port, " is busy, try to open the port " + ++this.port);
        continue;
      }
      this.listening = true;
    }
  ...
  _reqResp(request, response) {
    ...
    logger.log("Request '", request.method, "' received !");
    switch (request.method) {
      ...
      case HttpClient.functionType.connect:
        ...
        break;
      default:
        logger.warn("Unknown HTTP request method: ", request.method);
    }
  }
  ...
  _on_download(request, response, isOnlyHead, beforeMethod, afterMethod) {
    ...
    let pn = urlObj.pathname.substr(1)
    let retCode = 0;
    if (beforeMethod)
    ...
    logger.log("Get request for ", pn, ", type: ", contentType ? contentType : "unknown");
    // Read the requested file content from file system
    if (pn && !isOnlyHead) {
      File.readFile(this.rootDir + pn, function (success, data) {
        ...
        if (success || data)
          if (typeof data === constants.types.STRING)
            response.write(data);
  ...
  _on_upload(request, response, beforeMethod, afterMethod, methodName) {
    ...
    if (methodName !== HttpClient.functionType.delete) {
      ...
      request.addListener("end", function () {
        // let dataParam = querystring.parse(postData);
        if (afterMethod)
        ...
        response.writeHead(retCode, { 'Content-Type': 'text/plain' });
        response.end();
        logger.log("Get user upload data successful ! url: +", request.url);
      }.bind(this));
    } else {
      if (afterMethod)
      ...
      response.writeHead(retCode, { 'Content-Type': 'text/plain' });
      response.end();
      logger.log("Resolve user's deleting request over ! url: +", request.url);
    }
  }

  static getContentTypeByPathname(pathname) {
    let ext = pathname.split('.')[pathname.split('.').length - 1];
    return contentType[0][ext] ? contentType[0][ext] : contentType[1];
  }
}
